...
public interface Gfs025HourlyRepository extends CrudRepository<Gfs025Hourly, Long> {
    ...
    @Query(value = "SELECT * FROM gfs025_hourly a " +
            " WHERE a.station_code = ?1 " +
            " AND a.time > CURDATE() + INTERVAL ?2 day AND a.time < CURDATE() + INTERVAL ?2 + 1 DAY " +
            " AND a.updated_time = ( SELECT MAX(a.updated_time) FROM gfs025_hourly a " +
    ...
    @Query(value = "SELECT * FROM gfs025_hourly a WHERE a.station_code = ?1 " +
            " AND a.updated_time = ( SELECT max(updated_time) FROM gfs025_hourly a " +
            " WHERE a.station_code = ?1 AND updated_time < ?2 )", nativeQuery = true)
    List<Gfs025Hourly> getLast(String stationCode, Date day);
}
